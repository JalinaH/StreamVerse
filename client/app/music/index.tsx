...
export default function MusicListScreen() {
  ...
  const { music, status } = useSelector((state: RootState) => state.data as DataState);
  ...
  const handlePress = (item: Item) => {
    router.push({ pathname: '/music/[id]', params: { id: item.id } } as never);
  };
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        {status === 'loading' && music.length === 0 ? (
          ...
          <FlatList
            ...
            ListHeaderComponent={<Text style={styles.title}>Popular Music</Text>}
            ListEmptyComponent={<Text style={styles.emptyText}>No tracks available.</Text>}
          />
        )}
      </SafeAreaView>
    </GradientBackground>
