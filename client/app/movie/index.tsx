...
export default function MovieListScreen() {
  ...
  const { movies, status } = useSelector((state: RootState) => state.data as DataState);
  ...
  const handlePress = (item: Item) => {
    router.push({ pathname: '/movie/[id]', params: { id: item.id } } as never);
  };
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        {status === 'loading' && movies.length === 0 ? (
          ...
          <FlatList
            ...
            ListHeaderComponent={<Text style={styles.title}>Trending Movies</Text>}
            ListEmptyComponent={<Text style={styles.emptyText}>No movies available.</Text>}
          />
        )}
      </SafeAreaView>
    </GradientBackground>
